...
void cacheinit(hog_* descriptor, HCache_* cache, myMat* img, size_ paddingTL, size_ paddingBR, bool useCache, size_ cacheStride)
{
    ...
    cache->count1 = cache->count2 = cache->count4 = 0;
    for( j = 0; j < blockSize.width; j++ )
        for( i = 0; i < blockSize.height; i++ )
        {
            PData_* data = 0;
...
void detect(hog_* obj, myMat* img, point_** hits, double** weights, double hitThreshold, size_ winStride, size_ padding)
{
    ...
    cacheinit(obj, &cache, img, padding, padding, nwindows == 0, cacheStride);
					  ...
					  size_ winStride, size_ padding, double scale0, double finalThreshold, bool useMeanshiftGrouping)
{
	...
	veccopydouble(&tempScales, &foundScales);
	/*
	...
	*/	
	veccopyrect_(&allCandidates, foundLocations);
	/*
	...
	*/
	veccopydouble(&tempWeights, &foundWeights);
    groupRectangles(foundLocations, (int)finalThreshold, 0.2, 0, 0);
	sbfree(foundWeights);
	sbfree(levelScale);
